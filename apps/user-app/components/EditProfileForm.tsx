...
const FileInput: React.FC<{
  ...
  return (
    <div>
      ...
      <label htmlFor={id} className={`${fileInputBaseClass} ${error ? 'border-red-500' : 'border-slate-600'}`}>
        <span className={hasFile ? 'text-green-400' : 'text-slate-400'}>
          {hasFile ? `✅ ${displayedName}` : '파일 선택'}
        </span>
        <div className="px-3 py-1 text-xs font-semibold text-slate-300 bg-slate-600 rounded-md">
          {hasFile ? '변경' : '업로드'}
        </div>
      </label>
...
export const EditProfileForm: React.FC<EditProfileFormProps> = ({ initialProfile, onSubmit, onCancel, isLoading }) => {
    ...
    const commonInputClass = "w-full bg-slate-700/50 border border-slate-600 rounded-md px-3 py-2.5 text-slate-100 placeholder-slate-500 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition disabled:bg-slate-800 disabled:text-slate-500 disabled:cursor-not-allowed";

    return (
        <form onSubmit={handleSubmit} noValidate className="animate-fadeIn space-y-8">
            <div>
                <h2 className="text-2xl font-bold text-slate-100">회원 정보 수정</h2>
                <p className="text-slate-400 mt-1">수정할 정보를 입력하고 저장해주세요.</p>
            </div>

            {/* Basic Info */}
            <div className="bg-slate-900/50 p-5 rounded-xl border border-slate-700 space-y-5">
                <h3 className="text-lg font-semibold text-slate-200">기본 정보</h3>
                <div>
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1">주민등록번호 (외국인등록번호)</label>
            ...
            {/* Contact & Area */}
            <div className="bg-slate-900/50 p-5 rounded-xl border border-slate-700 space-y-5">
                 <h3 className="text-lg font-semibold text-slate-200">연락처 및 선호 지역</h3>
                 <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1">연락처</label>
                    <input type="tel" value={initialProfile.phone} className={commonInputClass} readOnly disabled />
                 </div>
                 <div>
            ...
            {/* Bank & Documents */}
            <div className="bg-slate-900/50 p-5 rounded-xl border border-slate-700 space-y-5">
                 <h3 className="text-lg font-semibold text-slate-200">계좌 및 서류</h3>
                 <div>
            ...
            <div className="mt-8 flex justify-end items-center gap-4">
                <button type="button" onClick={onCancel} className="px-6 py-3 text-sm font-medium bg-slate-600 hover:bg-slate-500 text-slate-200 rounded-md transition">
                    취소
                </button>
