...
export const Authentication: React.FC<AuthenticationProps> = ({ phone, onVerify, onBack, rememberMe }) => {
    ...
    return (
        <section className="bg-slate-800/50 p-6 md:p-8 rounded-xl shadow-2xl border border-slate-700 animate-fadeIn">
            <button onClick={onBack} className="flex items-center gap-2 text-sm text-slate-400 hover:text-amber-400 transition mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
                </svg>
                번호 다시 입력
            </button>
            <h2 className="text-2xl font-bold mb-2 text-center text-slate-100">인증번호 입력</h2>
            <p className="text-slate-400 mb-6 text-center">
                <span className="font-semibold text-amber-400">{maskedPhone}</span>(으)로 전송된<br/>6자리 인증번호를 입력해주세요.
            </p>
            <p className="text-center text-xs text-slate-500 mb-4">(테스트용 인증번호는 <span className="font-bold text-slate-300">{MOCK_OTP}</span> 입니다)</p>
