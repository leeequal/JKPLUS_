...
export const SiteDetail: React.FC<SiteDetailProps> = ({ site, onBack, appliedSiteId, onApply, currentUser }) => {
  ...
  const renderApplicationArea = () => {
    ...
    return (
      <button
        onClick={handleApply}
        disabled={isLoading}
        className="w-full md:w-auto px-10 py-4 font-semibold bg-amber-600 hover:bg-amber-500 text-white rounded-md transition transform hover:scale-105 disabled:bg-slate-700 disabled:cursor-not-allowed flex items-center justify-center mx-auto"
      >
        {isLoading ? (
          <>
  ...
  return (
    <div className="bg-slate-800/50 p-6 md:p-8 rounded-xl border border-slate-700 animate-fadeIn">
      <button 
        onClick={onBack}
      ...
      <header className="mb-6">
        <h2 className="text-2xl md:text-3xl font-bold text-slate-100">{site.name}</h2>
        <a 
          href={site.mapUrl} 
          target="_blank" 
          rel="noopener noreferrer"
          className="flex items-center gap-1.5 text-slate-400 mt-2 hover:text-amber-500 transition"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-slate-700/50 p-4 rounded-lg flex items-center gap-4">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-amber-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
          </svg>
          <div>
            <p className="text-slate-400 text-xs">일급</p>
            <p className="font-bold text-xl text-white">{site.dailyRate.toLocaleString()}원</p>
          </div>
        </div>
        <div className="bg-slate-700/50 p-4 rounded-lg">
          <div className="flex justify-between items-center mb-1 text-xs">
            <span className="text-slate-400">모집 현황</span>
            <span className="font-semibold text-slate-200">{site.filledSlots} / {site.totalSlots}</span>
          </div>
          <div className="w-full bg-slate-600 rounded-full h-2">
            <div className="bg-gradient-to-r from-amber-500 to-orange-500 h-2 rounded-full" style={{ width: `${vacancyPercentage}%` }}></div>
          </div>
          <p className="text-right text-xs mt-1.5 text-amber-300">{remainingSlots}자리 남음</p>
        </div>
        <div className="bg-slate-700/50 p-4 rounded-lg flex items-center gap-4">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-amber-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div>
            ...
            <p className="font-semibold text-lg text-white">{site.supervisor.name}</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div>
          <h4 className="font-semibold text-slate-200 mb-2">업무 내용</h4>
          <p className="text-slate-400 text-sm whitespace-pre-line">{site.description}</p>
        </div>
        
        <div className="bg-slate-900/50 p-5 rounded-lg border border-slate-700">
          <h4 className="font-semibold text-slate-200 mb-3">출근 확정 시 안내사항</h4>
          <dl className="space-y-3">
            <div>
              <dt className="flex items-center text-sm font-medium text-amber-400">
              ...
              <dd className="mt-1 ml-5 text-sm text-slate-300">{site.confirmationDetails.startTime}</dd>
            </div>
            <div>
              <dt className="flex items-center text-sm font-medium text-amber-400">
              ...
              <dd className="mt-1 ml-5 text-sm text-slate-300">
             ...
             <div>
              ...
              <dd className="mt-1 ml-5 text-sm text-slate-300">{site.confirmationDetails.contactInfo}</dd>
            </div>
            {site.confirmationDetails.notes && (
               <div>
                ...
                <dd className="mt-1 ml-5 text-sm text-slate-300">{site.confirmationDetails.notes}</dd>
              </div>
            )}
          </dl>
        </div>
